...
for step in range(MAX_STEPS):
    ...
    smoothness_loss = total_variation_loss(
        counterfactual_pixels
    )
    ...
    with torch.no_grad():
        ...
        target_probability = current_probs[
            0,
            TARGET_CLASS
        ].item()
    ...
    if (
        ...
        print(
            ...
            f"NORMAL: {current_probs[0, 0].item():.4f} | "
            f"PNEUMONIA: {current_probs[0, 1].item():.4f}"
        )


    # Stop after target is confidently reached
    if (
...
print(
    f"Fraction of pixels changed > 0.05: "
...
counterfactual_image.save(
    "outputs/counterfactual_smooth_image.png"
)
...
plt.savefig(
    "outputs/counterfactual_smooth.png",
    dpi=300
)
...
print(
    "\nSaved: outputs/counterfactual_smooth_image.png"
)

print(
    "Saved: outputs/counterfactual_smooth.png"
)